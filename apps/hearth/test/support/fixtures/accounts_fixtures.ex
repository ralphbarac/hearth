defmodule Hearth.AccountsFixtures do
  @moduledoc """
  This module defines test helpers for creating
  entities via the `Hearth.Accounts` context.
  """

  import Ecto.Query

  alias Hearth.Accounts
  alias Hearth.Accounts.Scope
  alias Hearth.Households

  def unique_user_email, do: "user#{System.unique_integer([:positive])}@example.com"
  def unique_username, do: "user#{System.unique_integer([:positive])}"
  def valid_user_password, do: "hello world!"

  def valid_user_attributes(attrs \\ %{}) do
    Enum.into(attrs, %{
      email: unique_user_email(),
      username: unique_username(),
      password: valid_user_password()
    })
  end

  def household_fixture(attrs \\ %{}) do
    {:ok, household} =
      attrs
      |> Enum.into(%{
        name: "Test Household #{System.unique_integer([:positive])}",
        features: %{"calendar" => true, "budget" => true, "grocery" => true}
      })
      |> Households.create_household()

    household
  end

  def unconfirmed_user_fixture(attrs \\ %{}) do
    household = attrs[:household] || household_fixture()

    {:ok, user} =
      attrs
      |> Map.delete(:household)
      |> valid_user_attributes()
      |> Map.put(:household_id, household.id)
      |> Accounts.register_user()

    %{user | household: household}
  end

  def user_fixture(attrs \\ %{}) do
    user = unconfirmed_user_fixture(attrs)

    # Auto-confirm the user for tests
    user
    |> Hearth.Accounts.User.confirm_changeset()
    |> Hearth.Repo.update!()
    |> Map.put(:household, user.household)
  end

  def user_scope_fixture do
    user = user_fixture()
    user_scope_fixture(user)
  end

  def user_scope_fixture(user) do
    Scope.for_user(user)
  end

  def set_password(user) do
    {:ok, {user, _expired_tokens}} =
      Accounts.update_user_password(user, %{password: valid_user_password()})

    %{user | household: user.household}
  end

  def extract_user_token(fun) do
    {:ok, captured_email} = fun.(&"[TOKEN]#{&1}[TOKEN]")
    [_, token | _] = String.split(captured_email.text_body, "[TOKEN]")
    token
  end

  def override_token_authenticated_at(token, authenticated_at) when is_binary(token) do
    Hearth.Repo.update_all(
      from(t in Accounts.UserToken,
        where: t.token == ^token
      ),
      set: [authenticated_at: authenticated_at]
    )
  end

  def generate_user_magic_link_token(user) do
    {encoded_token, user_token} = Accounts.UserToken.build_email_token(user, "login")
    Hearth.Repo.insert!(user_token)
    {encoded_token, user_token.token}
  end

  def offset_user_token(token, amount_to_add, unit) do
    dt = NaiveDateTime.add(NaiveDateTime.utc_now(:second), amount_to_add, unit)

    Hearth.Repo.update_all(
      from(ut in Accounts.UserToken, where: ut.token == ^token),
      set: [inserted_at: dt, authenticated_at: dt]
    )
  end
end

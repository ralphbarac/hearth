defmodule Hearth.Accounts do
  ...
  alias Hearth.Accounts.{User, UserToken, UserNotifier}
  ...
  def get_user_by_email(email) when is_binary(email) do
    User |> Repo.get_by(email: email) |> Repo.preload(:household)
  end
  ...
  def get_user_by_email_and_password(email, password)
    ...
    user = User |> Repo.get_by(email: email) |> Repo.preload(:household)
  ...
  @doc """
  Sets up the first household and admin user in a single transaction.
  Used during first-run when the database is empty.
  """
  ...
  See `Hearth.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `Hearth.Accounts.User.password_changeset/3` for a list of supported options.
